...
const Wc3LibTester = artifacts.require('Wc3LibTester')

contract('Wc3Lib library', _accounts => {
  let Wc3Lib
  before(async () => {
    Wc3Lib = await Wc3LibTester.new()
  })
  describe('randomUniform(bytes32,uint256,uint8)', async () => {
    ...
    it('zero range fails', async () => {
      await truffleAssert.reverts(
        Wc3Lib.randomUniform.call(phenotype1, 0, 0),
        'VM Exception'
      )
    })
    it('variable result when called with same seed and range but different phenotype', async () => {
      let rnd1 = await Wc3Lib.randomUniform.call(phenotype1, 0, 255)
      let rnd2 = await Wc3Lib.randomUniform.call(phenotype2, 0, 255)
      assert(rnd1.toString() !== rnd2.toString())
    })
    it('variable result when called with same phenotype and range but different seed', async () => {
      let rnd1 = await Wc3Lib.randomUniform.call(phenotype1, 0, 128)
      let rnd2 = await Wc3Lib.randomUniform.call(phenotype1, 1, 128)
      assert(rnd1.toString() !== rnd2.toString())
    })
    it('same result when called with same parameters', async () => {
      let rnd1 = await Wc3Lib.randomUniform.call(phenotype2, 2, 4)
      let rnd2 = await Wc3Lib.randomUniform.call(phenotype2, 2, 4)
      assert.equal(rnd1.toString(), rnd2.toString())
    })
    it('performing full range test works', async () => {
      for (let j = 1; j < 256; j++) {
        await Wc3Lib.randomUniform.call(phenotype1, j, j)
      }
    })
    it('uniformity', async () => {
      let iters = []
